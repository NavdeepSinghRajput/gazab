...
import com.gazabapp.utils.AppPreferences;
...
public abstract class NetworkHandler<T> implements Callback<T> {
     ...
     * not)
     */
    public boolean handleResponse(Request request, ResponseBody response) {
        return false;
    }
    ...
    public final void onResponse(@NonNull Call<T> call, @NonNull Response<T> response) {
        ...
        if (isRequestSuccessful(response)) {
            if (!handleResponse(call.request(), (ResponseBody) response.body())) {
                if ( response.body() != null) {
                    Log.e(getClass().getSimpleName(), String.format("Request: (%s), Response not handled: (%s)",
                            call.request().url(),  response.body().toString()));
                }
            }
        } else {
            Throwable error;
            try {
                ...
                try {
                    ...
                    JSONArray array;
                    if (TextUtils.isEmpty(AppPreferences.getInstance(context).getPreferencesString(Constants.ERROR_LOG))) {
                        array = new JSONArray();
                        array.put(jsonObject);
                    } else {
                        String value = AppPreferences.getInstance(context).getPreferencesString(Constants.ERROR_LOG);
                        array = new JSONArray(value);
                        array.put(jsonObject);
                    }
                    AppPreferences.getInstance(context).savePreferencesString(Constants.ERROR_LOG, array.toString());
